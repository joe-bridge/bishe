...
class GaussFilterDenoise(Filter):

  def __init__(self, net, cfg):
    Filter.__init__(self, net, cfg)
    self.short_name = 'D'
    self.begin_filter_parameter = cfg.gauss_begin_param
    self.num_filter_parameters = 1

  def filter_param_regressor(self, features):
    kernel_size = tf.cast(tanh_range(self.cfg.min_kernel_size, self.cfg.max_kernel_size)(features[:, 0]), tf.int32)
    kernel_size += tf.math.floormod(kernel_size, 2) - 1
    return kernel_size
