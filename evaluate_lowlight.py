...
class YoloTest(object):
    ...
    def predict(self, image, image_name):
        # 对输入图像进行预处理，包括复制原始图像、获取图像的高度、宽度，以及将图像大小调整为模型的输入尺寸。
        # 将预处理后的图像输入模型进行推理，获取预测结果，包括三个尺度（small、medium、large）的目标框预测值（pred_sbbox、pred_mbbox、pred_lbbox），以及经过图像增强处理后的图像（image_isped）和增强参数（isp_param）。
        # 将三个尺度的目标框预测值拼接在一起，并通过后处理函数（postprocess_boxes）和非最大抑制函数（nms）进行目标框的后处理和筛选，得到最终的目标框（bboxes）。
        # 如果启用了图像增强（isp_flag 为真），则将增强后的图像转换为合法的像素值范围（0-255），并通过逆预处理函数（image_unpreporcess）将图像恢复到原始尺寸。否则，将输入图像转换为合法的像素值范围。
        ...
        with open(self.annotation_path, 'r') as annotation_file:
            for num, line in enumerate(annotation_file):
                ...
                if len(bbox_data_gt) == 0:
                    ...
                    # bbox_data_gt数组的前4列作为bboxes_gt数组的元素，第5列作为classes_gt数组的元素，用于表示真实框的位置和类别。
