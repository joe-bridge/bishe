...
def evaluate_bm(all_metrics):
    ...
    MOTA = (1 - (fp + missed + ids) / g) * 100  # *100百分比
    # recall = TP / (TP + FN) = # corrected boxes / # gt boxes
    recall = c / g * 100
    # precision = TP / (TP + FP) = # corrected boxes / # det boxes
...
def evaluate_tracking(sequences, track_dir, gt_dir):
    ...
    for seqname in sequences:
        track_res = os.path.join(track_dir, 'output', seqname + '.txt')
    ...
    with open(file=os.path.join(track_dir, 'evaluate_res.txt'), mode='w') as f:
...
def parse_args():
    ...
    parser.add_argument(
        '--seqmap', help='seqmap file', type=str, default='./utils/seqmap.txt')
    parser.add_argument(
        '--track', help='Tracking result directory', type=str, default='./experiments_lowlight/exp_58/track')
    parser.add_argument(
        '--gt', help='Ground-truth annotation directory', type=str, default='./data')
...
if __name__ == '__main__':
    ...
    # 结果分别为[IDF1, IDP, IDR, recall, precision, FAR, n_gt, MT, PT, ML, fp, missed, ids, FRA, MOTA, MOTP, MOTAL]
